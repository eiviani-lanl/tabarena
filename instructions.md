...
You can adjust the search space of the ELM by altering `tabarena/tabarena/tabarena/models/elm/generate.py`
...
First, change the path in `run_eval.py` to the output path generated by `run_setup_slurm_jobs.py`. (--output flag in the generated command)

Run `python run_eval.py --type {all|binary|multiclass|regression}`

The generated figures will reside in `evals/custom_elm/{all|binary|multiclass|regression}`
